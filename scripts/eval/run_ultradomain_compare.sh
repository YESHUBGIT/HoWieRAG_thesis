...
QUESTIONS_PATH="$HOWIE_ULTRADOMAIN_PROCESSED_DIR/questions.jsonl"
CHUNKS_PATH="$HOWIE_ULTRADOMAIN_INDEX_DIR/chunks.jsonl"

if [[ ! -f "$QUESTIONS_PATH" || ! -f "$CHUNKS_PATH" ]]; then
  printf 'Missing prepared UltraDomain files.\n' >&2
